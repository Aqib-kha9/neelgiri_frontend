...
export const OrderCard = ({ order }: OrderCardProps) => {
  ...
  const handleTrackShipment = () => {
    router.push(`/dashboard/tracking/portal?awb=${order.awbNumber}`);
  };

  const handleEditOrder = () => {
    if (order.status !== "not_scheduled" && order.status !== "draft" && order.status !== "booked") {
      toast.error("Edit Restricted", {
        description: "This order has already been processed and cannot be edited. Please contact support."
      });
      return;
    }
    toast.info("Edit Order", {
      description: "Order editing feature will be enabled in the next update."
    });
  };

  const handleGenerateInvoice = () => {
    window.open(`/print/invoice/${order.awbNumber}`, "_blank");
  };

  const handleDownloadLabel = () => {
    window.open(`/print/label/${order.awbNumber}`, "_blank");
  };

  return (
    <>
      <OrderDetailsModal open={showDetails} onOpenChange={setShowDetails} order={order} />
      <QRCodeModal open={showQR} onOpenChange={setShowQR} order={order} />
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" className="rounded-xl">
                      <DropdownMenuItem className="flex items-center gap-2 rounded-lg cursor-pointer" onClick={handleGenerateInvoice}>
                        <FileText className="h-4 w-4" />
                        Generate Invoice
                      </DropdownMenuItem>
                      <DropdownMenuItem className="flex items-center gap-2 rounded-lg cursor-pointer" onClick={handleEditOrder}>
                        <Edit className="h-4 w-4" />
                        Edit Order
                      </DropdownMenuItem>
                      <DropdownMenuItem className="flex items-center gap-2 rounded-lg cursor-pointer" onClick={handleDownloadLabel}>
